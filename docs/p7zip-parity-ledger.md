...
- CLI selection: `*` and `?` wildcard matching for list/extract/delete
  operands.
...
- CLI: p7zip-compatible wildcard matching for test/update, overwrite prompts,
  warning paths, and byte-for-byte listing text are still incomplete.
